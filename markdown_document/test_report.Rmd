...
max_bike_age <- get_trip(trip_age_andel, "max", "Cykel")*100
max_car_age <- get_trip(trip_age_andel, "max", "Bil")*100
max_bus_age <- get_trip(trip_age_andel, "max", "Buss")*100
...
mest_bil__syfte_andel <- max(trip_purpose_mode_andel_df$Bil)
minst_bil_syfte <- rownames(trip_purpose_mode_andel_df)[which.min(trip_purpose_mode_andel_df$Bil)]
minst_bil__syfte_andel <- min(trip_purpose_mode_andel_df$Bil)
...
```{r}
bike_share_text <- function(share_bike_men, share_bike_women) {
   if(share_bike_men<share_bike_women){
     text_ <- "Kvinnor cyklar i större utsträckning än män"
   }
  else if(share_bike_men>share_bike_women){
    "Män cyklar i större utsträckning än kvinnor"
  }
  else{"Kvinnor och män cyklar i samma utsträckning"}
}
```
...
Män och kvinnor gör ungefär lika många resor per person och dag, `r num_trips_gender ` respektive `r num_trips_women`. Män använder bil i högre utsträckning än kvinnor, `r share_care_men*100` procent jämfört med `r share_care_women*100` procent. Kvinnor färdas i störra utsträckning med kollektivtrafik än män `r share_transit_women`, respektive `r share_transit_men` procent. Kvinnor cyklar i större utsträckning än män, `r share_bike_women` procent respektive `r share_bike_men` procent.
...
Andelen cykelresor är högst i åldersgruppen `r max_bike_age$Ålder` år, där `r max_bike_age$Cykel` procent av resorna utförs med cykel. Andelen bilresor ökar med stigande ålder och är högst i ålderskategorin `r max_car_age$Ålder` år, där `r max_car_age$Bil` procent av resorna utfördes med bil. Den högsta andelen bussresor är i ålderskategorin `r max_bus_age$Ålder` år där `r max_bus_age$Bus` procent av resorna utförs med buss. 
