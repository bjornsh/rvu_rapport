...
rapportomrade <- c("Bålsta", "Bålsta Skärplinge") #"Uppsala län" #Sträng eller array över områden som inkluderas
rapportomrade_text <- "Bålsta" #"Region Uppsala"  #namn på området som skrivs i löptexten
rapportomrade_rubrik <- "Bålsta tätort" #"Region Uppsala" #namn på området som skrivs i rubriker
...
#Producerar brödtext.
text_fardmedelsfordelning <- paste(tolower(mest_frekvent_fardmedel)," är det vanligaste färdmedlet i ",rapportomrade_text," och står för ",round(mest_frekvent_andel*100)," procent av alla resor. ",sep="")
...
if (nrow(avvikande_vanligaste_färdmedel) > 0) {
  ...
  text_fardmedelsfordelning <- paste0(text_fardmedelsfordelning," Detta är också det vanligaste färdmedlet i varje enskild ",omradesniva_text,".")
}
...
if (length(man_storst) > 0) {
  text_fardmedel_kon_sammanfattning <- paste(text_fardmedel_kon_sammanfattning,"Bland män stod ",paste(tolower(man_storst), collapse = ' och '), " för en större del av resorna (",round(man_storst_andel*100)," %) än bland kvinnor (",round(trip_mode_gender_andel_df["Kvinna",man_storst]*100)," %). ", sep = "")
}
if (length(kvinna_storst) > 0) {
  text_fardmedel_kon_sammanfattning <- paste(text_fardmedel_kon_sammanfattning,"För kvinnor var ", ifelse((length(man_storst) > 0),"istället "," "),paste(tolower(kvinna_storst), collapse = ' och '), " vanligare än bland män (",paste(round(kvinna_storst_andel*100), collapse = ' respektive ')," % för kvinnor och ",paste(round(trip_mode_gender_andel_df["Man",kvinna_storst]*100), collapse = ' respektive ')," % för män).", sep = "")
}
if (length(lika_stora) == 3) {
...
## Antal resor
Antalet resor per dygn i `r (rapportomrade_text)` uppgick till `r round(num_trips_all$"Antal resor")`. I genomsnitt gjorde `r round(num_trips_all$"Andel personer som rest (%)")` procent åtminstone en resa per dag. Totalt transporterade de boende sig `r round(sum(reslangd_summa$Trafikarbete_totalt)/1000)` kilometer under en genomsnittsdag summerat över samtliga färdmedel.
...
```{r fardmedelsfordelning-diagram, echo = FALSE, results = "asis", fig.cap = "Resornas färdmedelsfördelning sett till antalet resor."}
...
```{r fardmedelsfordelning-km-diagram, echo = FALSE, results = "asis", fig.cap = "Resornas färdmedelsfördelning sett till antalet fordonskilometer."}
#Tabell till diagram
...
Ur undersökningen har de `r (antal_individer)` respondenter som bor i `r (rapportomrade_text)` och som svarat på enkäten mellan `r tolower(fran)` och `r tolower(till)` valts ut. Antalet respondenter per månad är redovisat i Tabell \@ref(tab:resp-over-tid-table).
...
I `r (rapportomrade_text)` var en typisk resa (medianen av alla resor) `r round(reslangd_summa_tot$Median/1000)` kilometer lång. Längst var resorna med `r tolower(langst_resor_fardmedel)`, med en typresa på `r round(langst_resor_fardmedel_varde/1000)` kilometer. Kortast var resorna med `r tolower(kortast_resor_fardmedel)`, med en typresa på `r round(kortast_resor_fardmedel_varde/1000)` km.

`r (mest_frekvent_fardmedel_km)` var det vanligaste färdmedlet med `r round(mest_frekvent_fardmedel_km_varde/1000)` fordonskilometer, vilket motsvarar `r round(mest_frekvent_fardmedel_km_andel*100)` % av det totala antalet fordonskilometrar för samtliga trafikslag för boende i `r (rapportomrade_text)`.

För boende i `r area_shortest_trips$Område` var en typisk resa särskilt kort (`r area_shortest_trips$Median` km). De längsta resorna återfanns i `r area_longest_trips$Område`, med en typresa på `r area_longest_trips$Median` kilometer.
I `r area_max_fk$Område` gjordes den största sammanlagda förflyttningen med `r round(area_max_fk_km) ` fordonskilometer, medan `r area_min_fk$Område` stod för det minsta antalet fordonskilometer (`r round(area_min_fk_km)` km)
...
`r round(100*antal_resor_inom/antal_resor_tot_filtrerade)` procent av resorna som boende i `r (rapportomrade_text)` gjorde, skedde inom `r (rapportomrade_text)`. Den vanligaste förekommande resan inom en `r omradesniva_text` skedde i  `r (max_inom)`, vilket motsvarar `r round(100*max_inom_varde)` procent av alla resor. Den vanligaste resan mellan två `r omradesniva_text_plural` var från `r (max_fran)` till `r (max_till)` motsvarande `r round(100*max_till_fran_varde)` procent av alla resor. I Tabell \@ref(tab:od-matrix-table) framgår de vanligaste reserelationerna inom `r (rapportomrade_text)` och dess andelar av det totala antalet resor. Observera att det finns ett bortfall på `r 100-round(100*antal_resor_tot_filtrerade/antal_resor_tot)` procent i underlaget på grund av att vissa resor saknar information om avrese- eller ankomstplats.
