...
```{r setup, include=FALSE}
  ...
  for (pack in required_packages_list){
    if(pack %in% installed.packages() == FALSE){install.packages(pack, repos = "http://cran.us.r-project.org")}
  }
...
resor_per_dag_man <- round(num_trips_gender[num_trips_gender$Grupp=="Man",]$`Resor per dag och person`, digits = 2)
resor_per_dag_kvinna <- round(num_trips_gender[num_trips_gender$Grupp=="Kvinna",]$`Resor per dag och person`, digits = 2)
...
Män och kvinnor gör ungefär lika många resor per person och dag, `r resor_per_dag_man` respektive `r resor_per_dag_kvinna`. Män använder bil i högre utsträckning än kvinnor, `r share_car_men` procent jämfört med `r share_car_women` procent. Kvinnor färdas i störra utsträckning med kollektivtrafik än män `r share_transit_women`, respektive `r share_transit_men` procent. Kvinnor cyklar i större utsträckning än män, `r share_bike_women` procent respektive `r share_bike_men` procent.


## Fördelning mellan ålderskategorier


Andelen cykelresor är högst i åldersgruppen `r max_bike_age$Ålder` år, där `r max_bike_age$Cykel*100` procent av resorna utförs med cykel. Andelen bilresor ökar med stigande ålder och är högst i ålderskategorin `r max_car_age$Ålder` år, där `r max_car_age$Bil*100` procent av resorna utfördes med bil. Den högsta andelen bussresor är i ålderskategorin `r max_bus_age$Ålder` år där `r max_bus_age$Buss*100` procent av resorna utförs med buss. 
```{r}
...
Andelen cykelresor är högst i åldersgruppen `r max_bike_age$Ålder`, där `r max_bike_age$Cykel*100` procent av resorna utförs med cykel. Andelen bilresor ökar med stigande ålder och är högst i ålderskategorin `r max_car_age$Ålder`, där `r max_car_age$Bil*100` procent av resorna utfördes med bil. Den högsta andelen bussresor är i ålderskategorin `r max_bus_age$Ålder`där `r max_bus_age$Buss*100` procent av resorna utförs med buss. 
```{r}
...
`r ifelse(which.max(trip_age_andel_df$Kollektivtrafik)==1,"De yngsta",paste("Åldersgruppen",mest_koll_alder))` åker i högre utsträckning med kollektivtrafiken än övriga grupper. `r ifelse(koll_sjunkande,"Kollektivtrafiksandelen sjunker sedan med stigande ålder","")`. `r ifelse(mest_koll_alder==mest_cykel_alder,"Det också","Däremot är det")` åldersgruppen `r mest_cykel_alder` som gör fler cykelresor än övriga grupper. Andelen bilresor `r ifelse(bil_okande,"ökar med stigande ålder och ","")` är högst i ålderskategorin `r mest_bil_alder`, där `r round(max(trip_age_andel_df$Bil)*100)` procent av resorna görs med bil.

### Färdsätt efter sysselsättning
I åldern 15–85 år är `r round(sys_omrade_totalt$"Förvärvsarbetande/egen företagare"*100)` procent förvärvsarbetande, `r round(sys_omrade_totalt$Studerande*100)` procent studerande, `r round(sys_omrade_totalt$Pensionär*100)` procent är pensionärer. `r mest_frekvent_fardmedel` är det vanligaste färdmedlet för samtliga grupper `r ifelse(nrow(avvikande_vanligaste_färdmedel)>0,paste("Förutom för",upprakning(rownames(avvikande_vanligaste_färdmedel),10)),"")`. Bil användes i störst utsträckning av gruppen `r tolower(mest_bil_syss)` och minst i gruppen `r tolower(minst_bil_syss)`. Kollektivtrafiksandelen var högst i gruppen `r tolower(mest_koll_syss)`. Störst andel cykelresor stod gruppen `r tolower(mest_cykel_syss)` för.
...
`r (mest_frekvent_fardmedel_km)` är det vanligaste färdmedlet med `r tolower(mest_frekvent_fardmedel_km)` fordonskilometer, vilket motsvarar `r round(mest_frekvent_fardmedel_km_andel*100)` % av det totala antalet fordonskilometrar för samtliga trafikslag för boende i `r (rapportomrade_text)`.
...
```{r travel_share-calc_, echo = FALSE, results = "asis"}
...
```{r od-matrix-extern-kommun, echo=FALSE, include = FALSE}
...
inom_omrade <- utpendling #%>%
  #filter(stop_kommun_namn == rapportomrade)
...
utpendling <- utpendling %>%
  ...
  arrange(desc(antal)) %>%
    bind_rows(
      utpendling %>%
        filter(!(stop_kommun_namn %in% top_n(utpendling,minsta_antal-1)$stop_kommun_namn)) %>%
        summarise_if(is.numeric, sum) %>%
...
```{r od-matrix-extern-table-kommun, echo = FALSE, result="asis"}
utpendling_barplot <- ggplot(utpendling, aes(factor(stop_kommun_namn, level = stop_kommun_namn), perc, fill = factor(stop_kommun_namn, level = stop_kommun_namn))) +
    geom_bar(stat = "identity") +
    rvu_theme() +
...
```{r od-matrix-extern-tatort, echo=FALSE, include = FALSE}
...
utpendling <- utpendling %>%
  ...
  bind_rows(
      utpendling %>%
        filter(!(stop_tatort_namn %in% top_n(utpendling,minsta_antal-1)$stop_tatort_namn)) %>%
        summarise_if(is.numeric, sum) %>%
...
```{r od-matrix-extern-table-tatort, echo = FALSE, result="asis"}
utpendling_barplot <- ggplot(utpendling, aes(factor(stop_tatort_namn, level = stop_tatort_namn), perc, fill = factor(stop_tatort_namn, level = stop_tatort_namn))) +
    geom_bar(stat = "identity") +
    rvu_theme() +
    coord_cartesian(xlim = c(1,nrow(utpendling))) +
    scale_fill_manual(values=rev(palett)) +
