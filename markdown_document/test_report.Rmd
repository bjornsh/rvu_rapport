...
output:
  ...
  # toc_title: "Innehållsföreteckning"
...
```{r setup, include=FALSE}
  ...
  "knitr", "kableExtra", "tidyverse", "readxl", "bookdown", "pxweb", "scales", "stringi", "leaflet", "sf", "mapview", "processx", "tinytex", "jsonlite", "htmltools", "webshot", "jsontools")
...
reslangd_barplot <- ggplot(reslangd_per_fardmedel, aes(factor(Färdmedel, level = ordning_fardmedel), perc_reslangd, fill = factor(Färdmedel, level = ordning_fardmedel))) +
    ...
    scale_fill_manual(values=rev(palett)) +
    scale_y_continuous(labels = scales::percent) +
    geom_text(aes(label=percent(perc_reslangd, 2)),size = 3, position=position_dodge(width=0.9), vjust=-0.25) + #etiketter
    labs(
        x = "Färdmedel",
...
kbl(svarsgrad_tabell, caption = "Antal svar jämfört med befolkningen (15-85 år) i respektive område", booktabs = T,   linesep = "",format.args = list( big.mark = " ")) %>%
  ...
  row_spec(nrow(svarsgrad_tabell), bold = T) %>% #gör Total-raden fetstilt
  row_spec(0, bold = T) %>% #gör rubrikraden fetstilt
  footnote(symbol = "Baseras på särskilt lågt antal respondenter.")
}
```
...
Bil valdes i störst utsträckning för ärendet `r tolower(mest_bil_syfte)` (`r round(mest_bil_syfte_andel*100)` %). För ärende `r tolower(minst_bil_syfte)` användes dock bil i lägst utsträckning (`r round(minst_bil_syfte_andel*100)` %). Istället gjordes många resor för det ärendet istället med `r tolower(istallet_for_bil)` (`r round(istallet_for_bil_andel*100)` %).
...
utpendling <- utpendling %>%
  #filter(!(stop_kommun_namn %in% rapportomrade)) %>%
  top_n(minsta_antal) %>%
  arrange(desc(antal)) %>%
...
utpendling_barplot <- ggplot(utpendling, aes(factor(stop_kommun_namn, level = stop_kommun_namn), perc, fill = factor(stop_kommun_namn, level = stop_kommun_namn))) +
   ...
   scale_y_continuous(labels = scales::percent) +
    ...
    labs(
        x = "Resmål",
        y = "Andel resor",
        title = "Andel utpendlingsresor till olika kommuner"
    )
...
#Filtrerar ut största och sorterar
utpendling <- utpendling %>%
  #filter(!(stop_tatort_namn %in% rapportomrade)) %>%
  top_n(minsta_antal) %>%
  arrange(desc(antal)) %>%
  mutate(perc = antal/sum(antal))#%>%
  # mutate(antal = antal/summa) %>%
...
utpendling_barplot <- ggplot(utpendling, aes(factor(stop_tatort_namn, level = stop_tatort_namn), perc, fill = factor(stop_tatort_namn, level = stop_tatort_namn))) +
   ...
   scale_y_continuous(labels = scales::percent) +
    ...
    labs(
        x = "Resmål",
        y = "Andel resor",
        title = "Andel utpendlingsresor till olika tätorter"
    )
